...
func initaugmentation(suspsignatures bool,suspips bool,suspdomains bool,suspmd5 bool,defendedassets bool,defendedservices bool,client *elastic.Client ) {
	...
	if suspips {
		result, err := client.Search().Index("augmentation").Query(elastic.NewMatchQuery("augtype.keyword", "suspips")).From(1).Size(2000).Do(context.TODO())
		if err != nil {
			fmt.Println(err)
		}
	...
	if suspdomains {
		result, err := client.Search().Index("augmentation").Query(elastic.NewMatchQuery("augtype.keyword", "suspdomains")).From(1).Size(2000).Do(context.TODO())
		if err != nil {
			fmt.Println(err)
		}
...
func main() {
	...
	initaugmentation(true,true,true,true,true,true,client)
						...
						if err != nil {
							...
							for scanner.Scan() {
								...
								if objmap["event_type"] == "tls" {
									ja3hash := fmt.Sprintf("%v",objmap["tls.ja3.hash"])
									...
									dnsrrname := fmt.Sprintf("%v",objmap[" dns.rrname"])
									...
									md5key := fmt.Sprintf("%v",objmap["md5"])
									...
									if exists{
										...
										dstip := fmt.Sprintf("%v",objmap["dst_ip"])
									...
									if objmap["defended"] == false {
										...
										if exists{
											...
											dstport := fmt.Sprintf("%v",objmap["dst_port"])
								...
								if objmap["suspected"] == false{
									hashvalue := objmap["dst_ip"]
									if hashvalue != "" {
										dstipvalue := fmt.Sprintf("%v", objmap["dst_ip"])
								...
								//value, exists := suspsigmap["05af1f5ca1b87cc9cc9b25185115607d"];
							...
							if err != nil {
								...
								fmt.Println("***********************************************************")
								fmt.Println("FileName : NUMRECORDS ",file.Name() , numrecords)
								fmt.Println("***********************************************************")
